...
headers = {
    ...
    Chrome/53.0.2840.71 Safari/537.36'
}
...
def threadsDoIt(args):
    count = args[0]
    queue = args[1]
    print "{0}:{1}".format(count, queue)
    ...
    urlThreads = [Thread(target=getUrl, name="UrlThread", args=[processQue, imgPath, headers, ]) for i in range(4)]
    for t in urlThreads:
        t.start()
    for t in urlThreads:
        t.join()
...
if __name__ == '__main__':
    ...
    #     queue.put(http_url)
    #
    # urlThreads = [Thread(target=getUrl, name="UrlThread", args=[queue, imgPath, headers,]) for i in range(1)]
    # for t in urlThreads:
    #     t.start()
    # for t in urlThreads:
    #     t.join()
    ...
    pool.map(threadsDoIt, [(x, q) for x in range(4)])


    print "processes running time is %s s."%(time.clock()-start)
